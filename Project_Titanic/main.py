import click
import yaml
from src.import_and_clean_data import  import_data, clean_data
from src.model import  train_model
from src.check import  check_model
@click.command()
...
def run_pipeline(config):
    with open(config, 'r') as file:
     ...
     check_model.check(decisiontree_model, X_test, Y_test,"Decision_Tree_scores") # last argument is directory to save scores
     check_model.check(svm_model, X_test, Y_test,"Svm_model_scores") # last argument is directory to save scores
     check_model.check(random_forest_model, X_test, Y_test,"Random_Forest_scores") # last argument is directory to save scores
